...
unsafe fn modify_by_address(address: usize) {
    ...
    // to a `u32` and that it is properly aligned and dereferenceable.
    // SAFETY: The caller must guarantee that `address` is a valid, non-null pointer
    let ptr = address as *mut u32;
    *ptr = 0xAABBCCDD; // 修改地址处的值为 `0xAABBCCDD`
}
